...
rangedSum Nil _ _ = 0
rangedSum (Node value Nil Nil) r l = if value >= min l r && value <= max l r then value else 0
rangedSum (Node value left right) r l 
 | value >= min l r && value <= max l r = value + rangedSum left r l + rangedSum right r l
 | otherwise = rangedSum left r l + rangedSum right r l